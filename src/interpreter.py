...
class BuiltInFunction(BaseFunction):
    ...
    def execute_think(self, exec_ctx):
        command = exec_ctx.symbol_table.get("command")
        ...
        if not isinstance(command, String):
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    "'think' must be followed by a string",
                    exec_ctx,
                )
            )

        try:
            print(f"Prompt to be sent to AI: {command.value}")
        except Exception as e:
            return RTResult().failure(
                RTError(
                    self.pos_start,
    ...
    execute_think.arg_names = ["command"]
...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "think"):
            res.register_advancement()
            self.advance()
            ...
            if not expr:
                self.reverse(res.to_reverse_count)
            return res.success(
                ThinkNode(expr, pos_start, self.current_tok.pos_start.copy())
            )
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "import"):
            ...
            if not module.value.endswith(".think"):
                module.value += ".think"
            res.register_advancement()
            self.advance()
